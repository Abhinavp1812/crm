...
export default function ImportRegistrationsPage() {
  ...
  async function handlePreview() {
    ...
    const res = await fetch("/api/admin/import/registrations/preview", {
      method: "POST",
      body: fd,
    });
    const data = await res.json();
  ...
  async function handleCommit() {
    if (!file || !selectedSheet) return;
    if (!confirm(`Import ${preview?.newCount} new + ${preview?.updateCount} updates?`)) return;
    setLoading(true);
    setError("");
    const fd = new FormData();
    fd.append("file", file);
    ...
    let data: CommitResult & { error?: string } = {} as CommitResult;
    try {
      const res = await fetch("/api/admin/import/registrations/commit", {
        method: "POST",
        body: fd,
      });
      const text = await res.text();
      try {
        data = text ? JSON.parse(text) : ({} as CommitResult);
      } catch {
        setLoading(false);
      ...
      setLoading(false);
      if (!res.ok) {
        setError(data.error || `Commit failed with status ${res.status}`);
        return;
      }
      setResult(data);
    } catch (e) {
  ...
  return (
    <main className="min-h-screen p-8 bg-gray-50">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-2xl font-bold mt-2 mb-6">Import Registrations</h1>
            ...
            >
              ...
              {sheets.map((s) => (
                <option key={s.name} value={s.name}>
                  {s.name} ({s.rowCount} rows, {s.headers.length} columns)
                </option>
              ))}
            </select>
            <button
        ...
        {preview && !result && (
          <div className="bg-white p-6 rounded-lg shadow mb-4">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <Stat label="New customers" value={preview.newCount} color="green" />
              <Stat label="Existing (will update)" value={preview.updateCount} color="blue" />
              <Stat label="Skipped" value={preview.skipCount} color="amber" />
            </div>
            {preview.errorCount > 0 && (
              <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded">
                <p className="text-sm font-medium text-amber-900">
                  ⚠️ {preview.errorCount} issue(s) found.
                </p>
                <button
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={loading || preview.newCount + preview.updateCount === 0}
        ...
        {result && (
          <div className="bg-white p-6 rounded-lg shadow mb-4">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <Stat label="New customers" value={result.newCount} color="green" />
              <Stat label="Updated" value={result.updateCount} color="blue" />
